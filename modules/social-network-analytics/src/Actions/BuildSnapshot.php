...
final readonly class BuildSnapshot
{
    ...
    {
        $metric = trim($metric);
        if (preg_match('/^[a-z0-9][a-z0-9_.-]{0,119}$/', $metric) !== 1) {
            throw new InvalidArgumentException('Metric names are invalid.');
        }
        ...
        if ($periodStart > $periodEnd) {
            throw new InvalidArgumentException('The metric period is invalid.');
        }
