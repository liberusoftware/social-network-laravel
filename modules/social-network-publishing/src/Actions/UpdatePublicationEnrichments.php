    ...
    public function handle(Profile $author, Publication $publication, array $data): Publication
    {
        ...
        DB::transaction(function () use ($publication, $mentions, $hashtags, $links, $data): void {
            ...
            foreach ($mentions as $profileId) {
                DB::table('social_publication_mentions')->insert(['publication_id' => $publication->getKey(), 'profile_id' => $profileId, 'created_at' => now(), 'updated_at' => now()]);
            }
            foreach ($hashtags as $tag) {
                DB::table('social_publication_hashtags')->insert(['publication_id' => $publication->getKey(), 'tag' => $tag, 'created_at' => now(), 'updated_at' => now()]);
            }
            foreach ($links as $link) {
                $url = is_array($link) ? ($link['url'] ?? '') : $link;
                if (! filter_var($url, FILTER_VALIDATE_URL) || strlen($url) > 2048) {
                    throw new InvalidArgumentException('Each publication link must be a valid URL.');
                }
            ...
            if (array_key_exists('poll', $data)) {
                ...
                if (count($options) < 2 || count($options) > 20) {
                    throw new InvalidArgumentException('Polls require between 2 and 20 options.');
                }
