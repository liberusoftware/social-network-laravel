...
final readonly class IndexResource
{
    ...
    public function handle(Profile $owner, array $attributes): DiscoveryIndex
    {
        ...
        if ($type === '' || ! Str::isUuid($id) || $body === '' || mb_strlen($body) > (int) config('social-network-discovery.max_text_length') || ! in_array($visibility, (array) config('social-network-discovery.visibility_states'), true)) {
            throw new InvalidArgumentException('The discovery index payload is invalid.');
        }
