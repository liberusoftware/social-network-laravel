...
final readonly class CreateReport
{
    ...
    public function handle(Profile $reporter, string $targetType, string $targetId, string $reason, ?string $details = null): ModerationReport
    {
        ...
        if ($targetType === '' || ! Str::isUuid($targetId) || trim($reason) === '') {
            throw new InvalidArgumentException('A report target and reason are required.');
        }$report = DB::transaction(fn (): ModerationReport => ModerationReport::query()->create(['id' => (string) Str::uuid(), 'reporter_profile_id' => $reporter->getKey(), 'target_type' => $targetType, 'target_id' => $targetId, 'reason' => $reason, 'details' => $details, 'state' => 'open']));
